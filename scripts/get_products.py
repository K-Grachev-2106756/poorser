"""@package get_products docstring
    Функция get_products() вызывается третьей. Вызов асинхронный.
    Используется для получения информации о характеристиках товаров.
    В параметры формируемого запроса передаём ID товаров, которые
    получили после отработки функции get_ids. Затем полученный список
    укладывается в файл Products.json в кодировке UTF-8.
"""
import requests
import json
