"""@package get_prices docstring
    Функция get_prices() вызывается асинхронно. Используется для 
    получения актуальных цен на товары, номера которых добываются 
    из ProductsIds.json, их стоимости по скидке и информации о 
    бонусных рублях. Формируется запрос, а затем полученный список
    укладывается в файл Prices.json.
"""
import requests
import json
