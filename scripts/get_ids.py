"""@package get_ids docstring
    Функция get_ids() вызывается асинхронно. Используется для 
    получения набора кодов товаров, информацию о которых мы 
    будем получать в дальнейшем. Формируется запрос, а затем 
    полученный список укладывается в файл ProductsIds.json.
"""
import requests
import json
