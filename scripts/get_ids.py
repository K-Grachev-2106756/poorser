"""@package get_ids docstring
    Функция get_ids() вызывается первой. Вызов асинхронный. 
    Используется для получения набора кодов товаров, информацию о
    которых мы будем получать в дальнейшем. Формируется запрос:
    прописываются куки-файлы, хеддеры и параметры, которые не являются
    «подозрительными» для браузера, что в совокупности с асинхронным
    вызовом дает программе возможность получать данные с сайта. Затем
    полученный список укладывается в файл ProductsIds.json в кодировке 
    UTF-8.
"""
import requests
import json
